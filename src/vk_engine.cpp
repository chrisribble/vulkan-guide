...
void VulkanEngine::init_sync_structures() {
    ...
    VkSemaphoreCreateInfo sempahoreCreateInfo = vkinit::semaphore_create_info();

    for (int i = 0; i < FRAME_OVERLAP; ++i) {
        VK_CHECK(vkCreateFence(_device, &fenceCreateInfo, nullptr, &_frames[i]._renderFence));

        VK_CHECK(vkCreateSemaphore(_device, &sempahoreCreateInfo, nullptr, &_frames[i]._swapchainSemaphore));
        VK_CHECK(vkCreateSemaphore(_device, &sempahoreCreateInfo, nullptr, &_frames[i]._renderSemaphore));
    }
...
void VulkanEngine::init_descriptors() {
    // create a descriptor pool that will hold 10 sets of 1 image each
...
void VulkanEngine::init_background_pipelines() {
    ...
    if (!vkutil::load_shader_module("./shaders/gradient_color.comp.spv", _device, &gradientShader)) {
        fmt::println("Failed to load gradient compute shader\n");
    }

    VkShaderModule skyShader;
    if (!vkutil::load_shader_module("./shaders/sky.comp.spv", _device, &skyShader)) {
        fmt::println("Failed to load sky compute shader\n");
    }
...
void VulkanEngine::init_triangle_pipeline() {
    ...
    if (!vkutil::load_shader_module("./shaders/colored_triangle.frag.spv", _device, &triangleFragShader)) {
        fmt::println("Failed to load triangle fragment shader");
    } else {
    ...
    if (!vkutil::load_shader_module("./shaders/colored_triangle.vert.spv", _device, &triangleVertexShader)) {
        fmt::println("Failed to load triangle vertex shader");
    } else {
...
void VulkanEngine::draw() {
    ...
    // set swapchain image to Present so we can show it on the screen
...
void VulkanEngine::draw_background(VkCommandBuffer cmd) {
    ...
    vkCmdDispatch(cmd, std::ceil(_drawExtent.width / 16.0), std::ceil(_drawExtent.height), 1);
}
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd) {
    ...
    // set dynamic viewpoer and scissor
    VkViewport viewport = {};
    viewport.x = 0;
    viewport.y = 0;
